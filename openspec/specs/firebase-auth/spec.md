...
## Purpose
Establishes user identity with Firebase Authentication and Google sign-in, guards every application route behind that identity, and restricts access to an allowlisted email. Entirely client-side — no server-side OAuth flow, no session cookies.
## Requirements
