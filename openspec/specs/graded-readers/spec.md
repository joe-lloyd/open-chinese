# graded-readers Specification

## Purpose
Gives the learner somewhere to meet vocabulary in running text rather than only on flashcards: short stories pinned to an HSK level, read with a pinyin toggle, an English toggle and per-word lookups, with words the user has never encountered highlighted and moved into their personal dictionary when a chapter is finished. Tracks completion per reader and overall.
