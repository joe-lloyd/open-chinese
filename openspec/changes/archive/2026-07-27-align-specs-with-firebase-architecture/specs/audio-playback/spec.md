...
**Reason**: The requirement is unreachable and one of its scenarios is contradicted by the shipped app. Auto-play is gated on "the targeted sub-skill is `audio`", but no code path ever selects a targeted sub-skill: the study flow is a fixed four-phase model, the review state's `lastSubskill` is hardcoded to `null` on the way in and `'meaning'` on the way out, and `intervalAudio` is accepted and ignored by both `deriveStatus` and `checkMastery`. The precondition can never be true, so the requirement can never be exercised. Meanwhile its "No auto-play for non-audio sub-skills" scenario asserts that audio must **not** play on a meaning or pinyin reveal, which is the opposite of what the app does — audio plays on every pronunciation reveal, and on every fail.

**Migration**: Replaced by `Audio plays automatically on pronunciation reveal and on fail`, which describes the two unconditional triggers that actually exist. The "No auto-play for non-audio sub-skills" scenario is deleted outright and has no replacement; there is no configuration or code path that suppresses auto-play. The `intervalAudio` field itself is unaffected by this change — the existing `srs-engine` requirements stating that it is not assessed and does not block mastery remain accurate.
