...
test('checkout rejects catalogue packs that are not currently offered', async () => {
  let calls = 0
  const handler = createCheckoutHandler({
    inspectConfiguration: () => ready,
    provider: () =>
  ...
  const response = await handler(
    new Request('https://openchinese.app/.netlify/functions/checkout', {
      method: 'POST',
      body: JSON.stringify({ sku: 'hsk-9' }),
    })
  )
  assert.equal(response.status, 400)
  assert.deepEqual(await response.json(), { error: 'sku_not_offered' })
  assert.equal(calls, 0)
})
